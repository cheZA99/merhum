...
public interface IDeceasedService
{
    ...
    Task<DeceasedResponse?> GetByIdAsync(int id, string? scopeToUserId);
    ...
    Task<StatusChangeResult> UpdateStatusAsync(int id, int statusId, string? note, string changedByUserId);
    Task<string?> UploadPhotoAsync(int id, IFormFile file);
    Task<bool> DeleteAsync(int id);
    Task<List<StatusHistoryResponse>?> GetStatusHistoryAsync(int id, string? scopeToUserId);
}
